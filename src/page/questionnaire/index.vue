<template>
  <div class="questionnaire">
    <div class="questionnaire-header">
      <div class="questionnaire-header__title">中小学项目学期前测</div>
    </div>
    <div class="questionnaire-tip">
      <p class="questionnaire-tip__content">该测试为单选或多选题，请根据题目描述，选择答案。 非常同意为4分，完全不同意为0分。 对于开放提问，你可以分享不超过140字的反馈。
      <br/>隐私声明： 个人答案不会以任何形式分享给学校和老师，只会以数据集成的方式汇总成报告给学校</p>
    </div>
      ...
      <ul class="question-list">
        <li class="question-list-item with-face">
          ...
          <div class="question-list-item__options">
            <div class="left">
              <ul>
                <li class="sub-options" v-for="(item,index) in satisfaction.options" :key="item.value">{{index+1+'.'+ item.label}}</li>
              </ul>
            </div>
            <div class="right">
              <ul>
                <li class="sub-options" v-for="(item,index) in satisfaction.options" :key="index">
                  <span @click="handleSatisClick(item.id,0)" class="width-70"><img class="smileface" :src="item.checked === 0 ? smile : unsmile" alt=""></span>
                  <span @click="handleSatisClick(item.id,1)" class="width-42 margin-20"><img class="smileface" :src="item.checked === 1 ? smile : unsmile" alt=""></span>
                  <span @click="handleSatisClick(item.id,2)" class="width-28 margin-39"><img class="smileface" :src="item.checked === 2 ? smile : unsmile" alt=""></span>
                  <span @click="handleSatisClick(item.id,3)" class="width-28 margin-45"><img class="smileface" :src="item.checked === 3 ? smile : unsmile" alt=""></span>
                  <span @click="handleSatisClick(item.id,4)" class="width-56 margin-33"><img class="smileface" :src="item.checked === 4 ? smile : unsmile" alt=""></span>
                </li>
              </ul>
            </div>
          </div>
        ...
        <li class="question-list-item width-column">
          ...
          <div class="question-list-item__options">
            <el-row v-for="item in selection1.options" :key="item.value">
              <el-col :span="24" :class="{'is-active': selection1.value === item.value}" class="sub-options" @click.native="handleSelection1Change(item.value)">
                <i class="el-icon-circle-check-outline"></i><span class="option-title">{{item.label}}</span>
              </el-col>
            </el-row>
          </div>
        </li>
          ...
          <div class="question-list-item__options">
            <el-row>
              <el-col
                ...
                v-for="item in selection2.options" :key="item.value"
    ...
    <div class="submit-btn"><button>提交</button></div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    handleSatisClick(id, index) {
      let itemIndex = this.satisfaction.options.findIndex(item => item.id === id)
