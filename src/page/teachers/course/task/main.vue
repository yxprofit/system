    ...
    <aside :class="{'active':isNavShow}">
      ...
      <div>
        <a :class="{'active':current==1,'over': over==1}" @click="toCurrent(1, '/student/task/received')" @mouseenter="addover(1)" @mouseleave="addover(0)">
          <span><img :src="t9" v-if="current==1"><img :src="t9" v-else></span><span>收到的任务</span>
        </a>
        ...
        <a :class="{'active':current==3,'over': over==3}" @click="toCurrent(3, '/student/task/sponsor')" @mouseenter="addover(3)" @mouseleave="addover(0)">
          <span><img :src="t7" v-if="current==3"><img :src="t6" v-else></span><span></span><span>发起的任务</span>
        </a>
      </div>
    </aside>
    <section>
